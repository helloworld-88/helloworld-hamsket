Ext.define('HelloWorld.view.main.MainController', {
	...
	onTabChange(tabPanel, newTab, oldTab) {
		...
		if (newTab.id === 'helloworldTab') {
			if (HelloWorld.app.getTotalNotifications() > 0) {
				document.title =
					'HelloWorld (' + HelloWorld.app.getTotalNotifications() + ')';
			} else {
	...
	onRenameService(editor, e) {
		...
		Ext.getCmp('tab_' + e.record.get('id')).setTitle(
			Ext.String.htmlEncode(e.record.get('name')),
		);
	},
	...
	removeServiceFn(serviceId, total, actual, resolve) {
		...
		if (!rec.get('enabled')) {
			const session = rsession.fromPartition(
				`persist:${rec.get('type')}_${serviceId}`,
			);
		...
		if (config.default_service === rec.get('id'))
			ipc.send(
				'setConfig',
				Ext.apply(config, { default_service: 'helloworldTab' }),
			);
	...
	removeService(gridView, rowIndex, colIndex, col, e, rec, rowEl) {
		...
		Ext.Msg.confirm(
			...
			locale['app.window[13]'] +
				' <b>' +
				Ext.String.htmlEncode(rec.get('name')) +
				'</b>?',
			function (btnId) {
				if (btnId === 'yes') {
				...
				.replaceAll({
					fn(record) {
						...
						if (
							...
							return false;
						return Ext.String.htmlEncode(record.get('name'))
							.toLowerCase()
	...
	lockHelloWorld(btn) {
		...
		function showTempPass() {
			const msgbox = Ext.Msg.prompt(
				locale['app.main[19]'],
				locale['app.window[22]'],
				function (btnId, text) {
					if (btnId === 'ok') {
						const msgbox2 = Ext.Msg.prompt(
							locale['app.main[19]'],
	...
	showLockWindow() {
		...
		const validateFn = function () {
			if (
				...
				HelloWorld.util.MD5.encypt(winLock.down('textfield').getValue())
			) {
