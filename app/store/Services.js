		...
		load(store, records, successful) {
			...
			store.each(function (service) {
				...
				const cfg = {
					...
					includeInGlobalUnreadCounter: service.get(
						'includeInGlobalUnreadCounter',
					),
			...
			switch (config.default_service) {
				case 'last':
					Ext.cq1('app-main').setActiveTab(
						localStorage.getItem('last_active_service'),
					);
					break;
				case 'helloworldTab':
					break;
