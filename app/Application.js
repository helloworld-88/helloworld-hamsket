Ext.define('HelloWorld.Application', {
	...
	stores: ['ServicesList', 'Services', 'OS'],
	...
	launch() {
		...
		Ext.Loader.loadScript({
			url: Ext.util.Format.format(
				...
				(localStorage.getItem('locale') || 'en').replace('-', '_'),
			),
		});
		...
		if (
			...
			HelloWorld.app.checkUpdate(true);
	...
	checkUpdate(silence) {
		...
		Ext.Ajax.request({
			...
			success(response) {
				...
				const appVersion = new Ext.Version(
					require('@electron/remote').app.getVersion(),
				);
					...
					Ext.cq1('app-main').addDocked({
						...
						items: [
							...
							},
							{
								...
								href:
									'https://github.com/helloworld-88/helloworld-hamsket/releases/tag/' +
									updateVersion,
							},
							'->',
							{
