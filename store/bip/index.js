...
const parseElement = async element => {
  ...
  if (className && className.includes) {
    ...
    if (className.includes("Rkhuk3uDH-S1+56Lm78gIQ==")) {
...
const responseMessageHandler = () => {
  try {
    if(!document.querySelector('span[helloworld-id="helloworld-responseMessage"]')){
      ...
      let element = document.querySelector("div[data-name='MessageEditor']");
      if(element) element.insertAdjacentElement("afterbegin", span);
    }
  } catch (e) {
    console.error(e);
  }
      ...
      if (evt.key === "Enter") {
        ...
        if (!window.sendLock) {
          if(!helloworld.sendButton){
            let button = document.querySelector("div[class*='tedabDyyXsG-']");
            if(button){
...
const receiveMessagesHandler = async element => {
  try {
    ...
    let textElement = element.childNodes[0].childNodes[0].childNodes[0];
    if (textElement !== null) {
      ...
      if (!isEmpty(message) && !isNumber(message)) {
        ...
        if (!element.querySelector("div[class='helloworld-translation-message _79CqVlgnkUxGIdO8UVE-jg==']")) {
          div = document.createElement("div");
          div.className = "helloworld-translation-message _79CqVlgnkUxGIdO8UVE-jg==";
        } else {
          div = element.querySelector("div[class='helloworld-translation-message _79CqVlgnkUxGIdO8UVE-jg==']");
        }
        div.style.cssText = "display:block;color:var(--helloworldColor);font-size:var(--helloworldSize);";
...
window.setInterval(updateBadge, 3000);0;
